...
export default function MapView({ latitude, longitude, routes }) {
    ...
    (async () => {
      ...
      myMap.on("load", () => {
        ...
        if (
          ...
          sortedRoutes[0].polyline &&
          sortedRoutes[0].polyline.length > 0
        ) {
  ...
  useEffect(() => {
    ...
    if (myMap.getStyle && myMap.getStyle()) {
      try {
        ...
        console.log("Layers not ready yet for update." + e);
      }
    }
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {sortedRoutes.length > 0 && (
        <div
          className="absolute top-6 left-1/2 -translate-x-1/2 z-10 breathe-card px-6 py-4 flex flex-col gap-3 w-[min(90vw,420px)] shadow-xl"
          style={{ border: "1px solid rgba(255,255,255,0.1)" }}
        >
          <div className="flex justify-between items-center mb-1">
            <label className="text-sm font-semibold text-breathe-text-primary">
          ...
          <div className="flex justify-between text-xs font-medium text-breathe-text-secondary mt-1 options-labels">
            <span className={sliderIndex === 0 ? "text-[#007bff]" : ""}>Polly Rotues</span>
            {sortedRoutes.length > 2 && (
              <span
                className={sliderIndex === 1 ? "text-[#007bff]" : ""}
            ...
            {sortedRoutes.length > 1 && (
              <span
                className={
                  sliderIndex === sortedRoutes.length - 1 && sliderIndex !== 0
                    ? "text-[#007bff]"
                    : ""
                }
              >
